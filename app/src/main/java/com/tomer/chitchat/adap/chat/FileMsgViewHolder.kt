...
class FileMsgViewHolder(
    private val b: MsgItemFileBinding,
    ...
    private val onCli = View.OnClickListener {
        when (it.id) {
            ...
            b.msgTv.id, b.imgFileType.id -> callBack.onChatItemClicked(
                bindingAdapterPosition,
                ClickEvents.FILE
            )
        }
    ...
    override fun bind(mod: UiMsgModal) {
        ...
        b.msgTv.visibility = View.VISIBLE
        b.imgFileType.visibility = View.VISIBLE
        b.imgFileType.setImageDrawable(
            ContextCompat.getDrawable(
                context, AdapPerson.getDrawableId(mod.mediaFileName ?: "FILE")
            )
        )
        "${mod.mediaFileName ?: "File"}\n${mod.mediaSize} • ${
            Utils.getFileExt(mod.mediaFileName ?: "").uppercase()
        }".also { b.msgTv.text = it }
